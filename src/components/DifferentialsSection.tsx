...
const DifferentialsSection = () => {
  const differentials = [
    ...
    },
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 text-engverse-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <section id="diferenciais" className="py-24 bg-[#151E2C]">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {["Engenharia Civil", "Infraestrutura", "Indústria", "Mineração", "Geologia"].map((item) => (
                      <span 
                        key={item} 
                        className="inline-block bg-white/10 backdrop-blur-sm text-white text-sm px-4 py-2 rounded-full hover:bg-white/20 transition-colors"
                      >
                        {item}
                      </span>
                    ))}
          ...
          <div className="lg:col-span-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              {differentials.map((item, index) => (
                <div key={index} className="bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 p-6 hover:border-white/20 transition-all">
