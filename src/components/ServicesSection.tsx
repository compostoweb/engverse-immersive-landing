...
const ServiceCard: React.FC<ServiceCardProps> = ({ title, description, icon, imageUrl, accentColor }) => (
  <div className="group relative overflow-hidden bg-white/5 backdrop-blur-md rounded-2xl border border-white/10 hover:border-white/20 transition-all duration-500">
    <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500" 
         style={{background: `linear-gradient(to right, ${accentColor}10, transparent)`}}></div>
...
const ServicesSection = () => {
  const services = [
    {
      ...
      icon: (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-7 w-7 text-engverse-orange" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <section id="serviços" className="py-24 bg-[#0F172A]">
